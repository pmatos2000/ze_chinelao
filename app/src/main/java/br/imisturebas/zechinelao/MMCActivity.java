...
public class MMCActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < total; i++){
            text = new TextView(MMCActivity.this);
        ...
        btn_mais.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                total++;
                TextView text = new TextView(MMCActivity.this);
                text.setText("Entre com o " + total + "º número:");
                ll.addView(text);
                EditText edit = new EditText(MMCActivity.this);
                edit.setInputType(InputType.TYPE_CLASS_NUMBER);
                edit.setFilters(new InputFilter[]{new InputFilter.LengthFilter(9)});
                ll.addView(edit);
            }
        ...
        btn_menos.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if(total == 2) {
                    Toast.makeText(MMCActivity.this, "o calculo do MMC precisa de pelo menos 2 números", Toast.LENGTH_SHORT).show();
                }
                else {
        ...
        btn_solucao.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                for(int i = 0; i < total; i++){
                    ...
                    if(num == 0){
                        Toast.makeText(MMCActivity.this, "O " + (i+1) + "º tem que ser diferente de zero", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    u[i] = num;
                }
                Intent intent = new Intent(MMCActivity.this, SolucaoActivity.class);
                int a = (int) sp.getSelectedItemId();
                if(a == 0){
                    intent.putExtra("solução", MMC.solucao_txt(u));
                }
                else if (a == 1){
                    intent.putExtra("solução", MMC.solucao_fatoracao_txt(u));
                }
                else{
                    if(total > 2){
                        Toast.makeText(MMCActivity.this, "O metodo rapido não aceita mais de 2 números", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    intent.putExtra("solução", MMC.solucao_rapido_tex(u[0], u[1]));
                }

                MMCActivity.this.startActivity(intent);
            }
        });
